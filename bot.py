...
# Функція для перевірки наявності користувача в базі даних
def ensure_user_exists(telegram_id, ref_code=None):
    ...
    if not user:
        # Створюємо нового користувача, якщо його немає, і зберігаємо реферальний код у referred_by
        cursor.execute(
...
def handle_referral_code(invited_user_id, ref_code):
    try:
        # Перевіряємо наявність референта за реферальним кодом
        ...
        if referrer:
            ...
            # Оновлюємо поінти та кількість друзів у референта
            cursor.execute(
            ...
            # Перевіряємо, чи існує новий користувач у базі, і додаємо його з реферальним кодом
            ensure_user_exists(invited_user_id, ref_code)

            # Оновлюємо поінти у запрошеного користувача і записуємо, хто його запросив
            ...
            cursor.execute(
                ...
                (invited_user_points, ref_code, str(invited_user_id))
            )
            connection.commit()
...
def send_welcome(message):
    ...
    # Кнопка для відкриття mini web app
    markup = types.InlineKeyboardMarkup()
    web_app = types.WebAppInfo("https://bostion-surospx31s-projects.vercel.app/")
